...
from chat.speech import speech
...
def send(request):
    ...
    if message == "": 
        return HttpResponse('Message was empty')
    else:
...
def test2(request):
    ...
    if text != "":
        ...
        d = x.strftime("%x")
        t = x.strftime("%X")
        new_message = Message.objects.create(value=text, user=username,user_message=True,date=d,time=t)
        new_message.save()
        text = cbot(text)
        new_message = Message.objects.create(value=text, user=username,bot_message=True,date=d,time=t)
        new_message.save()
